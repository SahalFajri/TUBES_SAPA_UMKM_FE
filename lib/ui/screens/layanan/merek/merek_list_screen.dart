...
import 'merek_form_screen.dart'; // Import Form Screen nanti
...
class _MerekListScreenState extends State<MerekListScreen> {
  ...
  void _loadData() {
    setState(() {
      _historyFuture = _merekService.getHistory();
    });
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      floatingActionButton: FloatingActionButton(
        onPressed: () async {
          // Arahkan ke form pendaftaran merek
          await Navigator.push(
            context,
            MaterialPageRoute(builder: (context) => const MerekFormScreen()),
          );
          _loadData(); // Refresh list setelah kembali
        },
        backgroundColor: AppColors.primary,
  ...
  Widget _buildEmptyState() {
    return Center(
      child: Column(
        ...
        children: [
          Icon(Icons.label_off_rounded, size: 80, color: Colors.grey[300]),
          const SizedBox(height: 16),
          Text(
            "Belum ada pendaftaran merek",
  ...
  Widget _buildMerekCard(Map<String, dynamic> item) {
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              ...
              _detailItem("Nama Merek", item['namaMerek']),
              _detailItem("Kategori", item['kategori']),
              _detailItem("Deskripsi", item['deskripsiMerek']),
              _detailItem(
                "Tanggal Pertama Digunakan",
                item['tanggalDigunakan'],
              ),
              const Divider(height: 32),

              _detailItem("Nama Pemilik", item['namaPemilik']),
              _detailItem("NIK Pemilik", item['nik']),
              _detailItem("Alamat Pemilik", item['alamatPemilik']),
              const SizedBox(height: 20),
              ...
              const SizedBox(height: 12),
              _buildDocTile("Logo Merek", item['logoMerek']),
              _buildDocTile("Surat Pernyataan", item['suratPernyataan']),
            ],
          ),
        ),
      ),
